...
export default function CalendarPage() {
  ...
  // "Now" is captured once per page view and refreshed every minute so past
  // items roll off and day labels stay accurate.
    ...
    const upcoming = posts
      .filter(
        (p) =>
          p.status === "scheduled" &&
          p.scheduledAt &&
          new Date(p.scheduledAt).getTime() >= cutoff,
      )
      .sort(
        (a, b) =>
  ...
  const label = (key: string) => {
    ...
    const tomorrow = new Date(nowMs + 86_400_000).toDateString();
    ...
    <Page>
      <Inner>
        ...
        {isLoading ? (
          ...
          <Empty>
            Nothing scheduled yet. Schedule a post from the composer.
          </Empty>
        ) : (
